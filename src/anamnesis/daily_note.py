...
Appends session summary blocks to the daily note after each session log write.
...
Configuration:
    ...
    ANAMNESIS_WIKILINK_PREFIX — wikilink folder prefix (default: vault dir basename + "/")
"""
...
# Resolve vault dir (same default as store.py)
_VAULT_DIR = Path(
    os.environ.get("ANAMNESIS_VAULT", str(Path.home() / "Documents" / "Anamnesis"))
)

# Daily note directory — defaults to a sibling "Daily Logs" folder next to the vault
DAILY_LOG_DIR = Path(
    os.environ.get("ANAMNESIS_DAILY_DIR", str(_VAULT_DIR.parent / "Daily Logs"))
)

# Wikilink prefix for session links (e.g. "Claude Sessions/" or "Anamnesis/")
WIKILINK_PREFIX = os.environ.get("ANAMNESIS_WIKILINK_PREFIX", _VAULT_DIR.name + "/")

# Find Obsidian CLI on $PATH, fall back to bare name
